...
function createConvModel() {
  ...
  // 커널 크기가 3픽셀인 16개의 필터사용, relu 활성화
  ...
  model.add(tf.layers.dense({ units: 64, activation: "relu" })); // 0이하인 값은 => 0
  model.add(tf.layers.dense({ units: 10, activation: "softmax" })); // 결과의 합이 1이 되도록 유도(확률 표현에 유용)
  return model;
}
